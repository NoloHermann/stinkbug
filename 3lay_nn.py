# -*- coding: utf-8 -*-
"""
Created on Thu Nov  3 15:09:24 2022
...
class dlnet:
    ...
    def backward(self):
        ...
        dLoss_Z2 = dLoss_A2 * self.dRelu(self.ch['Z2'])    
        dLoss_A1 = np.dot(self.param["W2"].T,dLoss_Z2)
        dLoss_W2 = 1./self.ch['A1'].shape[1] * np.dot(dLoss_Z2,self.ch['A1'].T)
        dLoss_b2 = 1./self.ch['A1'].shape[1] * np.dot(dLoss_Z2, np.ones([dLoss_Z2.shape[1],1])) 
                            
        #dLoss_Z1 = dLoss_A1 * self.dRelu(self.ch['Z1'])    
        dLoss_Z1 = dLoss_A1 * self.dLRelu(self.ch['Z1'])  
...
for i in range(len(mq2)):
    ...
    """
nn = dlnet(input_vectors,targets)
nn.gd(input_vectors,targets, iter = 15000)
